...
Map {
    ...
    function setVisibleRegion(region) {
        ...
        // is nothing to do.
        _map.visibleRegion = QtPositioning.rectangle(
    ...
    onGcsPositionChanged: {
        if (gcsPosition.isValid && allowGCSLocationCenter
                && !firstGCSPositionReceived && !firstVehiclePositionReceived) {
            ...
            if (QGroundControl.settingsManager.flyViewSettings.keepMapCenteredOnVehicle.rawValue
                    || !_activeVehicleCoordinate.isValid)
                center = gcsPosition
        }
    }
    ...
    MapQuickItem {
        ...
        sourceItem: Image {
            ...
            height: ScreenTools.defaultFontPixelHeight * (isNaN(
                                                              gcsHeading) ? 1.75 : 2.5)
    ...
    Item {
        ...
        VideoOutput {
            id: customVideoOutput
            source: customMediaPlayer
            anchors.fill: parent
        }
    }
} // Map
